...
> 当前版本 **v1.2.5**（versionCode 6）· [更新日志](#更新日志)
...
- **核心数据**：已完成 / 进行中 / 总记录 / 坚持天数，一行四个数字，标签在上、数字在下（只留数字，不放补充小字）
- **分类进度**：图鉴各分类的收集进度，一行五个小圆环 + 「查看全部」进图鉴
- **最近解锁**：最近 3 条做成卡片，带封面（配过图就显示那张图，没有就用标题首字）、名称、描述与稀有度；具体时间不在这里露面，点进详情才看
- 收尾一行「从 X 开始记录 · 已记录 N 天」
- **板块可定制**：除问候语外的几段都能在「设置 → 首页板块」里开关、拖拽排序（长按任意一行进入编辑模式）。默认只显示**人生进度 / 核心数据 / 分类进度**三段——「最近解锁」一次多出三张卡片、「记录起点」只有一行日期、「自定义图片」得先上传图，想吃掉那一屏就去设置里打开。全关掉时首页给一个直达入口，随时能恢复默认
...
- **分类颜色**：挂在「核心数据」那一行下面，点一下展开——给图鉴的每个分类挑一个圆环颜色（首页「分类进度」那五个圈用它），没挑过的跟随主题强调色；只影响显示
- **分类可以自己加**：新建成就 / 编辑成就时能给成就挑分类，也可以当场新建一个自己的分类；首页「分类进度」里用开关决定显示哪五个（最多五个）
...
- **首页与设置页的板块也是液态玻璃**：人生进度 / 核心数据 / 分类进度 / 最近解锁、设置页的小标题分组，以及设置里的每个子页（成就设置 / 首页板块 / 备份 / 提醒 / 数据安全），走同一条折射管线，折射的是背景图与遮罩那一层 —— 设了背景图时卡片边缘能看见画面被掰弯、带一点色散；没设背景图时折射看不出来，卡片仍是一块带高光与描边的奶玻璃
...
| 包 | 说明 |
| --- | --- |
| `LifeLedger-v1.2.5.apk` | 正式签名包，日常使用装这个 |
| `LifeLedger-v1.2.5-debug.apk` | 调试包，多一组「设置 → 开发者选项」（生成演示数据 / 清空成就数据），用来复现界面、重拍截图 |
